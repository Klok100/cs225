...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    unsigned int elements_length = (subroot->elements).size();
    if (first_larger_idx < elements_length) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    unsigned int subroot_elements_length = subroot->elements.size();
    ...
    if (subroot->is_leaf) {
        ...
        if (first_larger_idx < subroot_elements_length) {
